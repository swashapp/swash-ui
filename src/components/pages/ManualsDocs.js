...
Surf-Streamr is a browser extension that can capture user browsing activities and listen to events on web pages. Actually, Surf-Streamr is not just a browser extension and it can call APIs on behalf of the user and send this data to Streamr to monetize it.
`,`###  Surf-Streamr modules
Surf-Streamr is a modular extension and, it currently provides 6 important modules: Search engine module, Facebook, Amazon, Twitter, Youtube, and Surfing modules.

----
#### Search Engines
Search engine module supports 6 top search engines: Google, Bing, Yahoo, AOL, Ask, Baidu. In this module, the user can choose whether his searching queries to be streams to Streamr or not.
![](./images/search.png)

----
#### Facebook
The Facebook module is more complicated. This module can capture the user's data in three ways. First is through a web request listener. Second, an injected javascript to Facebook pages and last is through Facebook APIs. This module can capture the user's visited facebook pages. A user has searches on Facebook. User books, videos, televisions, pages, and other his/her interests can be captured using Facebook APIs. 
![](./images/facebook.png)
...
The most important challenge was about user privacy. Along with user concern about privacy, complying with standards like GDPR is a challenge for our product.
...
As we would like to provide user's privacy, we shouldn't send any identity-related data to Streamr. Some data buyers may like to communicate to the user after performing an analysis on the user's data. Creating a communication channel which provides user's privacy is another challenge we faced during the design phase.

Most of the data capturing mechanism in our product depends on the way that the target web site has been designed and any changes in their site will affect our product. So by considering the limitation of browser extensions, designing the product in a way that it would update itself automatically, was another challenge. 

One other most important challenges associated with this project was data authenticity and integrity. Distinguishing a fake generated data from real user's data is very difficult and in some scenarios, it may be impossible.

To tackle these challenges, we came up with some mechanisms with the help of Streamr team and community.
`,`###  Privacy model
...
The first column of the table shows different data types that we have identified so far and the first row of the table shows privacy levels that we have defined for our product. 
At the lowest level, we assure no privacy. 
...
as we said before, data buyer may like to have a way to communicate with data owner without user privacy violation. We provide two mechanisms for this purpose. One is a simple mechanism using an email address that does not reveal user identity. 
...
`,`###  Surf-Streamr engine
The picture shown in this below, summarizes our idea about Surf-Streamr engine.
![](./images/engin.png)

At the lowest layer, each module gathers a specific type of data and passes it to the User Consent Policy layer. The second layer checks whether this data complies with user consent policy. If data complies, the extensoin forward it to the upper layer. In the third layer data filters give users the ability to exclude some domains and URLs from their data being collected. In the upper layer User, Privacy Level applies to data and based on the privacy level that has been chosen by the user, the only data that complies with the policy level will be passed to the Streamr.
...
Then, by using Surf-Streamr, the user gets paid for every data he provides to sell, but giant data collectors never share revenue of selling data.
Next, Surf-Streamr never reveals user identity unless the user gives permission, but user identity is known for the giant data collector.
Moreover, we are not limited to data regarding a specific business and we can collect a combination of user's data that none of these companies have all these data together.
As we are on the user side, we can collect some other interesting data that giant data collectors do not have access.
...
|User will not get paid for his own data|Users get paid for every data they provide to sell|
|User identity is known for giant data collectors|We never reveal user identity unless they give permission to do so|
