...
class OnBoardingJoin extends React.Component {
  ...
  constructor(props) {
    ...
    window.helper.generateJWTToken().then(token => this.setState({token}));    
    window.onmessage = this.handleMessages;
  }

  handleMessages(event){
    if (event.origin !== SWASHDOMAIN || !event.data)
      return;
    ...
    switch(code) {
      ...
      case 400:
        ...
        if(this.generalTryCount > MAXGENERALTRYCOUNT)
          this.reloadIFrame();
        break;
      case 401:
        this.tokenTryCount++;
        if(this.tokenTryCount > MAXTOKENTRYCOUNT)
      ...
      <div className="d-flex justify-content-center">
        <React.Fragment>
          <div className="onboarding-iframe-wrapper">
              {this.state.token ? <iframe seamless 
